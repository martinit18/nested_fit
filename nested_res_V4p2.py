...
current_version=4.2 # and beyond
...
class Analysis(object):
    ...
    def load_output_results(self, path):
        ...
        self.par_name = [p.replace("'", "") for p in self.input_data['par_name']]
    ...
    def load_output_data(self, path=currentpath):
        ...
        self.df = self.df.rename(columns={'val_%d' % np : self.input_data['par_name'][np-1].replace("'", "") for np in self.input_data['par_n']})
...
########################################################################################
    def read_input(self,path=currentpath):
        ...
        input_data['par_n']      = [int(lines[16+index].split()[0]) for index in range(npar)]
        input_data['par_name']   = [lines[16+index].split()[1] for index in range(npar)]
        input_data['par_value']  = [float(lines[16+index].split()[2]) for index in range(npar)]
        input_data['par_step']   = [float(lines[16+index].split()[3]) for index in range(npar)]
        input_data['par_min']    = [float(lines[16+index].split()[4]) for index in range(npar)]
        input_data['par_max']    = [float(lines[16+index].split()[5]) for index in range(npar)]
        input_data['par_fixed']  = [int(lines[16+index].split()[6]) for index in range(npar)]
...
#################################################################################################################
    def histo(self,par_number,path=currentpath,bins=50,plotmode='sigma',logbase=50.,xmin=None,xmax=None,savedata=False,alpha=1.,clear=True):
        ...
        if type(par_number) == str:
            ...
            print("Set par_number %s to %s" % (par_number, par_index))
            title = self.input_data['par_name'][par_number-1]
...
############################################################################################
    def plot_clusters(self,par_number1,par_number2,label=None,xmin=None,xmax=None,ymin=None,ymax=None,path=currentpath):
        ...
        if type(par_number1) == str:
            ...
            print("Set par_number %s to %s" % (par_number1, par_index1))
            title1 = self.input_data['par_name'][par_number1-1]
        ...
        if type(par_number2) == str:
            ...
            print("Set par_number %s to %s" % (par_number2, par_index2))
            title2 = self.input_data['par_name'][par_number2-1]
...
############################################################################################
    def plot_clusters3D(self,par_number1,par_number2,par_number3,label=None,
        ...
        if type(par_number3) == str:
            ...
            print("Set par_number %s to %s" % (par_number3, par_index3))
            title3 = self.input_data['par_name'][par_number3-1]
...
class Summary(object):
    ...
    def add_simulations(self, directories,labels,prefix=""):
        ...
        for dir in directories:
            ...
            par_tmp = par_tmp + [p.replace("'", "") for p in input_par['par_name']]
            #print par_tmp
        parameters = []
        ...
        inputs_par = ["par_n", "par_name","par_min","par_max","par_fixed"]
        ...
        for dir in directories:
            ...
            par_tmp = [p.replace("'", "") for p in input_par['par_name']]

            # From inputs
            for k, v  in input.items():
                if k in inputs_par:
                    for iv, vv in enumerate(v):
                        dir_input[k+'_'+par_tmp[iv]] = vv
                else:
                    dir_input[k] = v
            ...
            for k in input_types:
                if k in inputs_par:
                    for p in parameters:
                        summary[k+'_'+p].append(dir_input[k+'_'+p] if k+'_'+p in dir_input else np.nan)
                else:
                    summary[k].append(dir_input[k] if k in dir_input else np.nan)

            # From outputs
            for k, v  in output.items():
                if k in outputs_par:
                    for iv, vv in enumerate(v):
                        dir_output[k+'_'+par_tmp[iv]] = vv
                elif k !=  'ntry':
                    dir_output[k] = v
