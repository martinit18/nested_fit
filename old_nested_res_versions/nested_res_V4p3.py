...
current_version=4.3 # and beyond
...
########################################################################################
    def read_input(self,path=currentpath):
        ...
        # Convergence method
        input_data['conv_method'] = lines[5].split()[0]
        input_comment['conv_method'] = lines[5][lines[5].find('#'):]
        # Evidence final accuracy
        input_data['evaccuracy'] = float(lines[6].split()[0])
        input_data['conv_par'] = lines[6].split()[1]
        input_comment['convergence'] = lines[6][lines[6].find('#'):]
        # Search method
        input_data['search_method'] = lines[7].split()[0]
        input_comment['search_method'] = lines[7][lines[7].find('#'):]
        # Fraction of parameter standard deviation and n of jumps, max iterations and groups of iterations
        input_data['search_par1'] = float(lines[8].split()[0])
        input_data['search_par2'] = float(lines[8].split()[1])
        input_data['maxtries'] = int(lines[8].split()[2])
        input_data['maxntries'] = int(lines[8].split()[3])
        input_comment['search_algorithm'] = lines[8][lines[8].find('#'):]
        # Cluster analysis: if it is active and its parameters
        input_data['cluster_yn'] = lines[9].split()[0]
        input_data['cluster_method'] = lines[9].split()[1]
        input_data['distance_limit'] = float(lines[9].split()[2])
        input_data['bandwidth'] = float(lines[9].split()[3])
        input_comment['cluster_algorithm'] = lines[9][lines[9].find('#'):]
        # Number of tries and maximuma number of steps per tries
        input_data['ntry'] = int(lines[10].split()[0])
        input_data['maxstep_try'] = int(lines[10].split()[1])
        input_comment['ntry_maxstep'] = lines[10][lines[10].find('#'):]
        # Name of the function choosen for the fit
        input_data['function_name'] = lines[11].split()[0]
        input_comment['function_name'] = lines[11][lines[11].find('#'):]
        # Left-Right asymmetry parameter
        input_data['lr'] = lines[12][:1].split()[0]
        input_comment['lr'] = lines[12][lines[12].find('#'):]
        # Additional data for convolution
        input_data['npoint'] = int(int(lines[13].split()[0]))
        input_data['nwidth'] = int(int(lines[13].split()[1]))
        input_comment['additional data'] = lines[13][lines[13].find('#'):]
        # Limit of the spectra used for the fit
        input_data['xmin'] = float(lines[14].split()[0])
        input_data['xmax'] = float(lines[14].split()[1])
        input_data['ymin'] = float(lines[14].split()[2])
        input_data['ymax'] = float(lines[14].split()[3])
        input_comment['min_max'] = lines[14][lines[14].find('#'):]
        # Number of parameter used for the fit
        npar = int(lines[15].split()[0])
        input_data['npar'] = npar
        input_comment['npar'] = lines[15][lines[15].find('#'):]
        # Parameter of the fit, names, limits and "fix" flag
        input_comment['parameters'] = lines[16]
        parameters = []
        for index in range(npar):
            parameters.append([int(lines[17+index].split()[0]),
                lines[17+index].split()[1],
                float(lines[17+index].split()[2]),
                float(lines[17+index].split()[3]),
                float(lines[17+index].split()[4]),
                float(lines[17+index].split()[5]),
                int(lines[17+index].split()[6])])
