...
c     Polynomial on x^n*y^m with n even and n,m<=4
      ...
      POLY_EVENX =x**4*y_4+x**2*y_2+y_0
...
c     One minimum for yc>thr and 2 (or more if p>1) for yc<thr 
      ...
      IF(yc.LE.0) THEN
         FABIAN=amp*((x-x0)*(x+x0))**(2*q)+bg
      ELSE
         FABIAN=amp*(x**2)**(2*q)+bg
      END IF
...
c     second gauss peak
      ...
      DOUBLE_GAUSS_BG = GAUSS(x,y,3,val1) + GAUSS(x,y,3,val2) + bg
