...
INTEGER(4) :: nentries=100000,  n2b, n3b, junk_size=5
REAL(8) :: J_st
REAL(8), ALLOCATABLE, DIMENSION(:,:) :: me_2b, me_3b
REAL(8), DIMENSION(100000) :: nuc=-1, prot =-1, neut = -1,Jays=-1, length=-1, element=-1, sp=-1, sp_n, sp_p
...
CONTAINS
   ...
   SUBROUTINE INIT_NSM_FULL()
   !nuc,J,length,element,sp,me_3b,me_2b,J_st,n2b,n3b
   INTEGER(4) :: junk_size=7, i
   CHARACTER ::  string*64
   ...
   ! Reads the data from the matrix elements file
      ...
      DO i=1, nentries
      !nuc(i), Jays(i), length(i), element(i), sp(i),
         READ(2,*, END=45)  neut(i), prot(i), Jays(i),length(i),element(i), sp_n(i), sp_p(i), me_2b(:,i),me_3b(:,i)
         !write(*,*) neut(i), element(i), i
      END DO 
      45 CLOSE (2)
   
   

   END SUBROUTINE INIT_NSM_FULL
   ...
   REAL(8) FUNCTION NSM_FULL(X,npar,val)
      ...
      INTEGER(4) :: N, Z,k, l, i,loc , cnt, col, el, ier, switch, total
      INTEGER(4) :: loc_gs, lgth_gs, siz_gs, lgth, compar
      INTEGER(4) :: nentries=100000, junk_size=5, siz
...
!write(*,*) N, Jinp, lgth_gs, ltemp_gs, siz_gs
      ...
      IF (switch==1) THEN
         write(*,*)  N,Z,Jinp,'Something goes wrong'
      END IF
