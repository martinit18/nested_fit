...
c     The value of 'amp' is the value of the surface-->volume below the curve-->surface
      ...
      REAL*8 x0, y0, amp, sigmax, sigmay, aux  ! 2D: added y and sigma
      ...
      sigmax= val(4)
      sigmay= val(5)
      aux = -(x-x0)**2/(2*sigmax**2)-(y-y0)**2/(2*sigmay**2)
c     Test of under of underflow first
      IF(ABS(aux).LT.700) THEN   ! 2D: added. 
         GAUSS = amp/(2*pi*sigmax*sigmay)*dexp(aux)  ! Corrected normalisation. amp is now a volume 
      ELSE
         GAUSS = 0.d0
      END IF
...
c     for the pure gauss peak
      ...
      GAUSS_BG = GAUSS(x,5,val1) + bg   ! 2D: Changed 3->5

c     Save the different components
      IF(plot) THEN
         WRITE(40,*) x, y, GAUSS_BG, GAUSS(x,5,val1), bg ! 2D: added y Changed 3->5 ASKKK
      END IF
